// Google reviews with graceful fallback — shared by /api/reviews (the
// homepage ticker) and the server-rendered /reviews page. Live data requires
// GOOGLE_PLACES_API_KEY + GOOGLE_PLACE_ID; otherwise a small curated sample
// renders so the page never looks broken.

export type Review = { author: string; rating: number; text: string; time?: string };
export type ReviewsPayload = { live: boolean; rating?: number; total?: number; reviews: Review[] };
...
export async function getReviews(): Promise<ReviewsPayload> {
  ...
  if (key && place) {
    try {
      ...
      if (reviews.length) return { live: true, rating: result.rating, total: result.user_ratings_total, reviews };
    } catch {
      /* fall through to sample */
    }
  }
  return { live: false, reviews: SAMPLE };
}
