...
export default function ProfileEditor({
  ...
  const [form, setForm] = useState({
    ...
    bio: artist.bio ?? "",
    public_note: artist.public_note ?? "",
    instagram_url: artist.instagram_url ?? "",
    venue_url: artist.venue_url ?? "",
  ...
  async function uploadPortrait(file: File) {
    ...
    if (up.error) {
      setBusy(false);
      setStatus(`Error: ${up.error.message}`);
      return;
    }
  ...
  async function addFlash(file: File) {
    ...
    const { error } = await supabase
      .from("flash")
      .insert({ artist_id: artist.id, image_url: pub.publicUrl, sort_order: flash.length });
    setBusy(false);
    setStatus(error ? `Error: ${error.message}` : "Image added.");
    router.refresh();
  }
  ...
  async function removeFlash(id: string) {
    ...
    setBusy(false);
    setStatus(error ? `Error: ${error.message}` : "Removed.");
    router.refresh();
  }
