...
export default function EstateApp({ artists }: { artists: Artist[] }) {
  ...
  // mouse parallax → CSS vars on the root
  useEffect(() => {
  ...
  // scroll-reveal within the active scene
  useEffect(() => {
    const root = rootRef.current; if (!root) return;
  ...
  function go(s: Scene) { setScene(s); setButlerTarget(LINES[s]); }
  function ring() { setDoorsOpen(true); setButlerTarget("Ah — a guest! Do come in. Allow me to take your coat..."); setTimeout(() => go("foyer"), 1750); }
    ...
    <div className="estate" ref={rootRef}>
      ...
      <section className={`scene ${scene === "foyer" ? "active" : ""}`}>
        ...
        <div className="scene-inner center">
          ...
          <div className="section-kicker reveal">Where shall I escort you?</div>
          <div className="filigree reveal"><span>❦</span></div>
          <div className="rooms">
            <div className="room-card reveal" onClick={() => go("artists")}><span className="room-emblem">🎨</span><div className="room-name">Hall of Portraits</div><div className="room-desc">Her Grace&rsquo;s artists</div></div>
            ...
            <div className="room-card reveal" onClick={() => go("booking")}><span className="room-emblem">✉</span><div className="room-name">Appointment Parlor</div><div className="room-desc">Request your sitting</div></div>
            <div className="room-card reveal" onClick={() => go("boutique")}><span className="room-emblem">👛</span><div className="room-name">Maison Baroness</div><div className="room-desc">The ritual line</div></div>
            <div className="room-card reveal" onClick={() => go("salon")}><span className="room-emblem">📜</span><div className="room-name">The Salon</div><div className="room-desc">The house &amp; how to call</div></div>
          </div>
        </div>
      </section>
      ...
      <section className={`scene ${scene === "artists" ? "active" : ""}`}>
        ...
        <div className="scene-inner">
          ...
          <div className="roomhead reveal"><div className="section-title">Hall of Portraits</div><div className="section-kicker">The artists in Her Grace&rsquo;s service</div></div>
          <div className="filigree reveal"><span>❦</span></div>
          {artists.length === 0 ? (
...
.estate .cream-bg{position:absolute;inset:-4%;background:radial-gradient(120% 90% at 50% 0%,#fbf3e2 0%,var(--cream) 45%,var(--cream-deep) 100%);transform:translate(calc(var(--px)*-14px),calc(var(--py)*-10px));transition:transform .25s ease-out}
...
@keyframes sceneIn{0%{opacity:0;transform:scale(1.03) translateY(10px)}100%{opacity:1;transform:none}}
.estate .scene-inner{position:relative;z-index:5;width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding:64px 30px 150px;overflow-y:auto;scroll-behavior:smooth}
...
.estate .rooms{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:20px;width:100%;max-width:1040px;perspective:1000px}
.estate .room-card{position:relative;background:linear-gradient(180deg,#fdf6e7,var(--cream-deep));border:1px solid var(--gold);border-radius:5px;padding:30px 22px 26px;text-align:center;cursor:pointer;transition:transform .35s cubic-bezier(.2,.7,.2,1),box-shadow .35s;box-shadow:0 10px 26px rgba(0,0,0,.14)}
...
.estate .room-card:hover{transform:translateY(-8px) rotateX(6deg) scale(1.02);box-shadow:0 26px 50px rgba(0,0,0,.28),0 0 0 1px var(--gold-light)}
.estate .room-emblem{font-size:38px;color:var(--gold-dark);margin-bottom:12px;display:block;transition:transform .4s}
.estate .room-card:hover .room-emblem{transform:translateY(-4px) scale(1.12)}
...
.estate .portrait{position:relative;background:#fdf6e7;border:1px solid var(--gold);border-radius:5px;overflow:hidden;cursor:pointer;transition:transform .35s,box-shadow .35s;box-shadow:0 10px 26px rgba(0,0,0,.14)}
.estate .portrait:hover{transform:translateY(-6px) scale(1.015);box-shadow:0 22px 44px rgba(0,0,0,.28)}
...
.estate .ware{position:relative;background:linear-gradient(180deg,#fdf6e7,var(--cream-deep));border:1px solid var(--gold);border-radius:6px;overflow:hidden;text-align:center;box-shadow:0 10px 24px rgba(0,0,0,.14);padding-bottom:26px;transition:transform .35s,box-shadow .35s}
.estate .ware:hover{transform:translateY(-6px);box-shadow:0 22px 44px rgba(0,0,0,.26)}
...
@media(max-width:560px){.estate .photos{columns:2 150px}.estate .flashgrid{grid-template-columns:repeat(2,1fr)}.estate .backline{top:18px;left:18px}.estate .contact{grid-template-columns:1fr}.estate .quarters{top:18px;right:20px}}
